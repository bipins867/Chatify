...
const buttonSendScheduleMessage = document.getElementById(
...
function addChat(chat, userMe = true, isLink = false, sender) {
  ...
  //div.className = userMe ? myMessageClass : otherMessageClass;
  if (userMe) {
...
async function onPageRefress(groupUID) {
  ...
  if (OBJ.id2Get == -1) {
    result = await axios.get(
      baseUrl + `Chat/getChat?lastChatId=${lastChatIndex}`,
      { headers }
    );
  } else {
    result = await axios.get(
      baseUrl + `Group/getChat/${OBJ.id2Get}?lastChatId=${lastChatIndex}`,
      { headers }
    );
  }
  //console.log(result)
...
buttonSendScheduleMessage.onclick = async (event) => {
  const currentTime = new Date(inputDatePicker.value);
  const currentMiliSec = currentTime.getTime();
  ...
  try {
    ...
    if (OBJ.id2Get == -1) {
      const result = await axios.post(baseUrl + "Chat/postChat", obj, {
        headers,
      });
    } else {
      const result = await axios.post(
        baseUrl + `Group/postChat/${OBJ.id2Get}`,
        obj,
        { headers }
      );
    }
...
buttonSend.onclick = async (event) => {
  try {
    if (inputMessage.value == "") return;
    const obj = { chat: inputMessage.value, isScheduled: false };
...
buttonCreateGroup.onclick = async (event) => {
  ...
  try {
    const result = await axios.post(baseUrl + "Group/createGroup", obj, {
      headers,
    });
...
buttonJoinGroup.onclick = async (evnet) => {
  ...
  try {
    const result = await axios.get(baseUrl + `Group/addGroup/${groupUID}`, {
      headers,
    });
