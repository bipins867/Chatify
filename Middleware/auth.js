...
exports.userTypeAuthenticate = async (req, res, next) => {
  ...
  try {
    const userGroupAdmin = await UserGroup.findOne({
      where: {
        UserId: req.user.id,
        GroupId: groupId,
        memberType: "admin",
    ...
    if (!userGroupAdmin) {
      return res
        .status(201)
        .json({ message: "You are not admin of the group" });
    }
    ...
    if (superAdminUser.createdBy != req.user.id) {
      if (userId == superAdminUser.createdBy) {
        return res
          .status(202)
          .json({ message: "You can't edit Super Admin User" });
      }
    }
    ...
    if (superAdminUser.createdBy == userId) {
      return res
        .status(203)
        .json({ message: "You are the Super Admin you can't edit yourself" });
    }
    next();
  } catch (err) {
    console.log(err);
